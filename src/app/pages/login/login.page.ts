...
import { BackgroundGeolocation, BackgroundGeolocationEvents, BackgroundGeolocationResponse } from '@awesome-cordova-plugins/background-geolocation/ngx';
...
export class LoginPage implements OnInit {
  ...
  login() {
    this.isLoading = true;
    this.authService.getAccess(this.form.value.Username, this.form.value.Password, this.UUid)
      .subscribe((res: LoginResponse) => {
        this.isLoading = false;
        if (res.hasbusinesserror == 1 && res.Message == "UserPassIncorrect") {
          this.sharedService.toast('danger', this.language.login.UserPassIncorrect);
          return
        }
        ...
        if (res.Equivalent_User_ID) this.getAccessByID(res.Equivalent_User_ID);
  ...
  getAccessByID(equivalentUserId: string) {
    this.authService.getAccessByID(equivalentUserId).subscribe((res: LoginResponse) => {
